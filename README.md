...
基于 [VLOOK™](https://github.com/MadMaxChow/VLOOK) 的 Typora 自定义主题，使用 thinking（思考）风格。
...
- **字号**：基础字号 18px
- **样式**：仅保留 thinking 主题，删除了其余 5 个主题及对应字体文件，减小体积
...
1. 将本仓库克隆到 Typora 主题目录：
   ...
   ```
2. 将 `vlook-thinking.css` 复制到主题目录上级：
   ```
   copy "%APPDATA%\Typora\themes\vlook\vlook-thinking.css" "%APPDATA%\Typora\themes\"
   ```
3. 重启 Typora，在「主题」菜单中选择 `Vlook-Thinking`

## 目录结构

```
├── vlook-thinking.css       # 主题入口文件（需复制到 themes/ 目录）
